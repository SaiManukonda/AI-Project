...
model1 = LogisticalRegression(X_Train, Y_Train, 0.00000001, training_data_size)
...
for i in range(int(testing_data_size)):
    currentWiring = WiringDiagram()
    flattenedFeatureArray = extractFeatures(currentWiring.diagram)
    prediction = model1.predict(flattenedFeatureArray)
    if prediction == currentWiring.dangerous:
        correct_cnt += 1
